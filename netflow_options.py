...
# 700 - 800 has been found to be good for smallers WISPs and medium-sized organizations
# ~200 has been found to be good for small organizations
# < 100 will create a large amount of index connections (with only a few flow records in each) and cloud affect performance
...
#
# After changing this port run the following commands from a root-evel shell:
...
# The Elasticsearcch host and port to send flow data to
elasticsearch_host = '127.0.0.1'

#### DNS Lookups ####
# The appliance will attemp to do a reverse DNS lookup for IP address reported in flows,
# in order to make correlation between traffic and different domains. Traffic to / from certain
# domains is also categorized based on domain name for higher-level reporting. The appliance
# must be pointed to an **INETRNAL** DNS server, which is able to resolve internal addresses, as
...
#
# Set to False if you don't want to enable DNS lookups, otherwise change to True to enable loogups.
dns = False
...
# If you have an internal DNS server that can do these lookups set to True, otherwise leave it set to False.
