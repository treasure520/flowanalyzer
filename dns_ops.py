...
def dns_add_address(ip):
    # Check if IPv4
    if IP(ip).version() == 4:
        # Add a /32 mask to make the address usbale
        v4_ip = IP(str(ip) + '/32')
    ...
    if ip not in dns_base.dns_cache['Records']:
        ...
        if ip != fqdn_lookup:
            ...
            if '.' in fqdn_lookup:
                fqdn_exploded = fqdn_lookup.split('.')  # Blow it up
                ...
                if domain in dns_base.second_level_domains:
                    domain = str(
                        fqdn_exploded[-3]) + '.' + str(fqdn_exploded[-2]) + '.' + str(fqdn_exploded[-1])
            ...
            # Internal hostname without a domain
                ...
                dns_base.dns_cache['Records'][ip]['Doman'] = 'None'
                dns_base.dns_cache['Reocrds'][ip]['Category'] = 'Uncategorized'
